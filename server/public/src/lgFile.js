/**
 * @description: 基于原生 js 和 input-file 的 h5/web 大文件上传插件，不依赖其他类库，引入即用，支持并发上传、切片上传，参数及样式可配置
 * @version: V0.0.1
 * @author: funnyque@163.com
 * @Github: https://github.com/funnyque/lagreFileUpload
 */
(function (win, doc) {
  // 插件默认配置
  var oConf = {
  ...
  function build() {
    var arr = [
      '<div class="lg-file">',
      '   <div class="btns">',
      '       <input type="file" name="file" class="file-inpt" accept="' +
        this.conf.accept +
        '"' +
        (this.conf.multiple ? " multiple " : "") +
        ' style="display: none;">',
      ...
      '       <input type="button" value="' +
        this.conf.btnU +
        '" class="upload no-border radius pointer white bg-blue">',
      '       <input type="button" value="' +
        this.conf.btnD +
        '" class="delete no-border radius pointer white bg-red">',
      '       <input type="button" value="' +
        this.conf.btnC +
        '" class="cancel no-border radius pointer white bg-red">',
    ...
    for (var i in this.files) {
      if (this.files[i] !== null) {
        var arr = [
          ...
          '   <div class="info">',
          '       <p class="over-hidden ' +
            matchColor(this.files[i].status) +
            '">',
          '           <span class="name-size">',
          '               <i class="f-size white radius ' +
            matchColor(this.files[i].status, 1) +
            '">' +
            this.files[i].size +
            "</i>",
          ...
          '           <div class="progress ' +
            matchColor(this.files[i].status, 1) +
            '" style="width:' +
            this.files[i].percent +
            ';"></div>',
          "       </div>",
          "   </div>",
          '   <div class="pic-box">',
          '       <i class="pic no-selec' +
            (/data:image\//.test(this.files[i].data)
              ? '" style="background-image: url(' + this.files[i].data + ');"'
              : ' icon"') +
            "></i>",
          "   </div>",
          ...
          '       <input type="checkbox" ' +
            ...
            ' data-id="' +
            i +
            '" class="checone pointer">',
          '       <i data-id="' +
            i +
            '" class="deleone no-selec pointer white"></i>',
          "   </div>",
          "</li>",
        ];
        arrs.push(arr.join(""));
  ...
  function packFiles(ofs) {
    ...
    if (ofs.length > this.conf.maxCount) {
      // 触发onMax事件
      this.onMax &&
        this.onMax.call(_this, {
          in: ofs.length,
          max: this.conf.maxCount,
        });
      for (var i = 0; i < this.conf.maxCount; i++) {
        fs.push(ofs[i]);
      }
    } else {
      fs = ofs;
    ...
    for (var i = 0; i < fs.length; i++) {
      ...
      frs[i]["readAs" + ra]
        ? frs[i]["readAs" + ra](fs[i])
        : frs[i].readAsDataURL(fs[i]);
    }
  }
  function upload() {
    var _this = this,
    ...
    for (var i in this.files) {
      if (
        ...
        this.xhrs[i].send(
          this.setData
            ? this.setData.call(this, this.files[i])
            : this.files[i].data
        );
        this.xhrs.on++;
        ...
        this.xhrs[i].onload = function () {
          ...
          if (
            flag == undefined ? this.status >= 200 && this.status < 300 : flag
          ) {
  ...
  function delet(flag, tag) {
    if (flag) {
      ...
      for (var i in this.files) {
        if (this.files[i] !== null && this.files[i]["checked"])
          this.files[i] = null;
      }
    }
    render.call(this);
      ...
      if (this.files[k] !== null && this.files[k]["checked"]) {
        ...
        if (this.xhrs[k]) {
          this.xhrs[k].onabort = function () {
            _this.xhrs.cdone++;
            if (_this.xhrs.con == _this.xhrs.cdone)
              showMsg.call(_this, "成功终止选中");
          };
          this.xhrs[k].abort();
        } else {
          _this.xhrs.cdone++;
